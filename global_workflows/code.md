...
# WORKFLOW: /code - The Universal Coder v4.0 (BMAD-Enhanced)
...
3. Git setup (Skill: git-workflow):
   - git init
   - Tạo .gitignore
   - **Branching strategy** (hỏi user):
     1️⃣ GitHub Flow (simple - dành cho solo/small team)
     2️⃣ Trunk-based (néu có CI/CD mạnh)
     → Mặc định: GitHub Flow nếu không hỏi
   - Tạo `.gitmessage` template (commit convention):
...
### 0.4. GitNexus Context Check (Tự động - AWF Integration) 🔬

Ngay sau khi xác định được project path, kiểm tra GitNexus index:

```
Nếu có .gitnexus/ trong thư mục project:
    → Kích hoạt skill awf-gitnexus-context
    → Báo: "🔬 GitNexus sẵn sàng — em có thể phân tích impact kiến trúc trước khi sửa code."
    → Trước khi sửa bất kỳ symbol/hàm quan trọng: ưu tiên chạy impact() để soi rủi ro

Nếu KHÔNG có .gitnexus/:
    → Báo 1 lần (không nhắc lại):
      "💡 Tip: Chạy `gitnexus analyze` trong project này để unlock phân tích kiến trúc sâu hơn."
    → Tiếp tục coding bình thường
```

### 0.5. UI/UX Pro Max Context Check (BẮT BUỘC khi task đụng UI)

Nếu task có bất kỳ thay đổi nào về giao diện, layout, component, form, dashboard, landing page, mobile screen, table, chart, navigation, animation, responsive, hoặc visual polish:

1. Đọc `docs/design-specs.md` nếu có.
2. Đọc `design-system/MASTER.md` nếu có.
3. Nếu đang làm một page cụ thể, đọc `design-system/pages/[page-name].md` nếu có.
4. Kích hoạt skill `ui-ux-pro-max` để kiểm tra style, layout, accessibility, interaction, responsive và anti-patterns.

**Thứ tự ưu tiên khi có conflict:**

```
1. design-system/pages/[page-name].md
2. design-system/MASTER.md
3. docs/design-specs.md
4. Existing app/component patterns
5. ui-ux-pro-max baseline recommendation
```

**Nếu thiếu toàn bộ design specs:**

```
Nếu user đang yêu cầu làm UI ngay:
→ Chạy ui-ux-pro-max baseline:
  python skills/ui-ux-pro-max/scripts/search.py "<product_type> <screen> <vibe>" --design-system -p "<Project Name>" -f markdown
→ Tạo design baseline tối thiểu trong docs/design-specs.md trước khi code UI.

Nếu user chưa rõ gu/UI:
→ Gợi ý chạy /visualize trước để chốt mockup và design specs.
```

**Không được code UI chỉ dựa trên cảm tính nếu task có yêu cầu production quality.**

### 0.2.1. Full Plan Execution (All Phases) ⭐ v4.0
...
**Trước khi sửa code hoặc báo kết quả, đọc `global_workflows/GLOBAL_SAFETY_TRUTHFULNESS_GATE.md`.**

- Chỉ nói test/build/lint pass nếu đã chạy thật.
- Nếu không chạy được, phải báo rõ là chưa chạy.
- Repo-observed fact phải dựa trên file/command đã đọc.
- Không tự ý thêm scope ngoài yêu cầu hoặc tự nhận đã hoàn thành thứ chưa verify.
...
**Nếu đã có mockup từ /visualize hoặc design specs từ ui-ux-pro-max, PHẢI tuân thủ:**

#### Source of Truth Checklist
```
□ Đã đọc docs/design-specs.md
□ Đã đọc design-system/MASTER.md nếu tồn tại
□ Đã đọc design-system/pages/[page-name].md nếu tồn tại
□ Đã kiểm tra ui-ux-pro-max rules liên quan đến task
□ Đã ghi nhận anti-patterns cần tránh
```
...
□ Dùng semantic theme tokens, không hardcode hex rải rác trong component
□ Font-family, font-size, font-weight đúng
□ Spacing (margin, padding) đúng
□ Border-radius, shadows đúng
□ Icon set, stroke width, icon size nhất quán
```
...
□ Focus state thấy rõ bằng keyboard
□ Disabled state không còn tương tác
```
...
□ Không có text overflow trong button/card/tab/table
□ Fixed header/sidebar/bottom nav không che content
```

#### E. Accessibility & No AI-Slop Gate
```
□ Contrast đạt WCAG AA
□ Form errors nằm gần field và có cách sửa rõ ràng
□ Không dùng màu làm tín hiệu duy nhất (phải có icon/text)
□ Hover/focus/animation không gây layout shift
□ Animations tôn trọng prefers-reduced-motion
□ Không dùng emoji làm structural icons
□ Không dùng palette một màu đơn điệu nếu design specs không yêu cầu
□ Không đặt card trong card trừ khi component system đã định nghĩa rõ
```
...
### 4.3.1. Test Skip Behavior (Khi chọn option 2) ⭐ v4.0
...
### 5.4. ⭐ LAZY CHECKPOINT SYSTEM (AWF 4.0)
...
## ⚠️ Anti-Rationalization (Những lý do bỏ qua bước — và tại sao SAI)

| Lý do bỏ qua | Thực tế |
|---|---|
| "Cái này đơn giản, không cần spec" | Task đơn giản không cần spec dài, nhưng vẫn cần acceptance criteria. |
| "Sẽ thêm test sau" | Test debt compound exponentially. Thêm ngay khi còn nhớ logic. |
| "Cứ code trước, clean up sau" | Clean up sau không bao giờ xảy ra. Code debt trở thành permanent debt. |
| "Thêm tính năng này tiện thể" | Scope creep làm chậm delivery. Chỉ làm những gì được yêu cầu. |
| "Refactor luôn cho đẹp" | Surgical changes. Chỉ sửa đúng chỗ cần. Refactor là task riêng. |
| "Skip validation, user sẽ không nhập sai" | Users nhập sai. Attackers nhập sai có chủ ý. Validate mọi input. |

---

## 🚩 Red Flags (Dấu hiệu AI đang làm sai — dừng ngay và check)

- Bắt đầu code mà chưa có requirements bằng văn bản
- Đang modify nhiều hơn 5 files cho một task nhỏ (scope quá lớn)
- Thêm tính năng không có trong spec hoặc task list
- Ra quyết định kiến trúc mà không document
- "Tiện tay" sửa code không liên quan đến task
- Test fail sau 3 lần fix mà không hỏi user
- Đang dùng `any` type hoặc tắt TypeScript errors
- Không có error handling cho external API calls
- Thay đổi database schema mà chưa hỏi user
- Deploy/push code mà không có sự cho phép rõ ràng
