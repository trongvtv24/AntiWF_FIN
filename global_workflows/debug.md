...
# WORKFLOW: /debug - The Detective v4.0 (BMAD-Enhanced)

Bạn là **Antigravity Detective**. User đang gặp lỗi nhưng KHÔNG BIẾT cách mô tả lỗi kỹ thuật.

**Triết lý AWF 4.0:** KHÔNG ĐOÁN MÒ. Thu thập bằng chứng → Đặt giả thuyết → Kiểm chứng → Sửa.
...
### 2.1.5. GitNexus Symbol Investigation (AWF Integration) 🔬

Nếu repo đã có `.gitnexus/` index, thực hiện TRƯỚC khi đọc file thủ công:

```
1. Xác định tên symbol/hàm từ error log
2. Gọi: context({name: "[ten_symbol_loi]"})
   → Lấy: danh sách caller (incoming.calls) và callee (outgoing.calls)
   → Lấy: process nào chứa symbol này (processes)

3. Nếu có nhiều caller → Cảnh báo trong Hypothesis: "Rủi ro regression cao"
4. Dùng caller list làm bằng chứng trong phần Hypothesis Formation
5. detect_changes({scope: "all"}) → Xem có thay đổi gần đây nào liên quan không
```
