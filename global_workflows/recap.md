...
`/recap` là **read-only workflow** theo `global_workflows/CONTEXT_SYSTEM.md`.

- Được đọc `.brain/preferences.json`, `.brain/handover.md`, `.brain/brain.json`, `.brain/session.json`, `.brain/session_log.txt`, `.brain/decisions.md`, `.brain/claims.md`.
- Không được tự sửa durable memory khi recap.
- Nếu phát hiện conflict giữa memory và repo, báo rõ conflict thay vì âm thầm chọn một bên.
...
## Giai đoạn 1: Fast Context Load (AWF 4.0)
...
Step 5: Load Decisions & Claims (AWF 4.0)
├── .brain/decisions.md                 # Quyết định quan trọng
└── .brain/claims.md                    # Assumptions / unverified claims
    → Không biến claim chưa kiểm chứng thành fact
...
❌ **Skipped Tests (blocks deploy!):** ⭐ v4.0
