...
### 1.0. Pre-Refactor Impact Check (GitNexus - AWF Integration) 🔬

**Thực hiện ĐẦU TIÊN trước khi bất cứ điều gì:**

```
Nếu có .gitnexus/ trong project:
    1. Nhận diện symbol/hàm/class cần refactor từ yêu cầu của anh
    2. Gọi: impact({target: "[ten_symbol]", direction: "upstream", minConfidence: 0.8})
    3. Đánh giá risk_level:
       - HIGH → BẮT BUỘC liệt kê toàn bộ điểm bị ảnh hưởng + XIN PHÉP anh trước
       - MEDIUM → Cảnh báo + cho anh xác nhận muốn tiếp tục
       - LOW → Mention nhẹ, tiếp tục refactor
    4. Với rename: dùng rename({symbol_name: "cu", new_name: "moi", dry_run: true})
       → Hiển thị preview tất cả file sẽ bị sửa trước khi thực thi

Nếu KHÔNG có .gitnexus/:
    → Tiếp tục bình thường (không block workflow)
    → Gợi ý: "gitnexus analyze" để có safety check tốt hơn lần sau
```
