...
Deploy là high-risk workflow. Trước khi deploy:

- Đọc `global_workflows/GLOBAL_SAFETY_TRUTHFULNESS_GATE.md`.
- Không nói app production-ready nếu build/test/security/preflight chưa pass.
...
## Giai đoạn 0: Pre-Audit Recommendation ⭐ v4.0
...
### 9.3. Deploy Strategy (Skill: deployment-patterns)

Hỏi user muốn deploy theo kiểu nào:

```
"🚀 Anh deploy theo cách nào?

1️⃣ **Direct Deploy** (Đơn giản - dành cho app nhỏ)
   - Build → Upload → Done
   - Phù hợp: Vercel, Netlify, Railway

2️⃣ **Blue-Green** (An toàn - không downtime)
   - Chạy song song bản cũ và mới
   - Chuyển traffic khi bản mới OK
   - Nếu lỗi → Quay về bản cũ ngay lập tức

3️⃣ **CI/CD Pipeline** (Tự động - mỗi push code là deploy)
   - GitHub Actions tự chạy test → build → deploy
   - Phù hợp: app đang phát triển liên tục

4️⃣ **Canary Release** (Thận trọng - thử với % nhỏ user trước)
   - 5% user thấy phiên bản mới
   - Nếu OK → rollout 100%"
```

**Nếu chọn CI/CD (option 3):** Tự động tạo `.github/workflows/deploy.yml` phù hợp với stack dự án.

### 9.4. Release Management (Skill: github-ops)

Sau khi deploy thành công, tự động thực hiện:

```
1. Tạo git tag version (semantic versioning):
   → Hỏi: "Đây là bản [patch/minor/major]?"
   → Auto tag: v1.0.0 → v1.0.1 (patch), v1.1.0 (minor)

2. Tạo GitHub Release:
   → Auto generate changelog từ commit messages
   → Tóm tắt những gì thay đổi trong bản này

3. Đóng các Issues liên quan:
   → Scan commit messages tìm "closes #XX"
   → Tự động close issues tương ứng
```

**Hỏi user:**
```
"Anh muốn em tạo Release trên GitHub không?
1️⃣ Có - Em tạo tag + Release notes + đóng issues liên quan
2️⃣ Không cần - Bỏ qua bước này"
```
...
## 🛡️ Resilience Patterns (Ẩn khỏi User) - v4.0
